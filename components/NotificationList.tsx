...
const NotificationList: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1b1b2b] via-[#24243a] to-[#151522] relative overflow-hidden px-4">
      {/* Background glow */}
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto space-y-6 pb-10 pt-px">
        {/* Search and Filter */}
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="relative w-full md:w-96">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
            <input type="text" placeholder="Search notifications..." className="w-full pl-10 pr-4 py-3 rounded-xl bg-[#2a2a40] border border-slate-700 text-white placeholder-slate-400 focus:ring-2 focus:ring-cyan-400 outline-none transition" value={search} onChange={(e) => setSearch(e.target.value)}/>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 w-full md:w-auto scrollbar-hide">
            {['ALL', ...Object.values(NotificationCategory)].map((cat) => (
              <button key={cat} onClick={() => setFilter(cat as any)} className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all border ${filter === cat ? 'bg-cyan-400 text-white border-cyan-400' : 'bg-[#23233a] text-slate-300 border-slate-700'}`}>{cat}</button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {filtered.length > 0 ? filtered.map((notif) => {
            ...
            return (
              <div key={notif.id} className={`bg-[#1f1f2e]/90 backdrop-blur-xl rounded-2xl border transition-all shadow-lg ${!isViewed && !isOwn && !user?.isPrincipal ? 'ring-2 ring-cyan-400 border-transparent' : 'border-slate-700 hover:border-cyan-400'}`}>
                <div className="p-6">
                  <div className="flex flex-col md:flex-row justify-between gap-6">
                    <div className="flex-1 space-y-4">
                      <div className="flex items-center gap-2">
                        ...
                        {notif.priority === Priority.HIGH && <span className="bg-rose-900 text-rose-300 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider animate-pulse">Critical</span>}
                        {!isViewed && !isOwn && <span className="w-2 h-2 bg-cyan-400 rounded-full"></span>}
                      </div>
                      <div>
                        <h3 className="text-xl font-bold text-white leading-tight cursor-pointer hover:text-cyan-400 transition-colors" onClick={() => { setSelectedNotif(notif); if(!isViewed && !isOwn) markAsViewed(notif.id); }}>{notif.title}</h3>
                    ...
                    <div className="md:w-56 flex flex-col justify-center">
                      {isOwn ? (
                        <button onClick={() => setSelectedNotif(notif)} className="w-full py-3 bg-[#23233a] text-cyan-400 border border-cyan-900 rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-cyan-950 transition-all">
                          <Users size={18}/> Tracking ({stats.viewed} Viewed)
                        </button>
                      ) : (
                        notif.requiresAcknowledgment ? (
                          <button onClick={() => handleAcknowledge(notif.id)} disabled={isAcked} className={`w-full py-3 rounded-xl flex items-center justify-center gap-2 text-sm font-bold transition-all ${isAcked ? 'bg-emerald-900 text-emerald-300 border border-emerald-800' : 'bg-cyan-400 text-white hover:bg-cyan-500 shadow-md active:scale-95'}`}>
                            {isAcked ? <CheckCircle2 size={18}/> : null}
                            {isAcked ? 'Acknowledged' : 'Acknowledge Notice'}
                          </button>
                        ) : (
                          <div className="text-center py-2 px-3 rounded-xl border border-slate-800 bg-[#23233a] text-[10px] font-bold text-slate-400 uppercase">Information Notification</div>
                        )
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-[#1f1f2e]/90 backdrop-blur-xl rounded-3xl border border-dashed border-slate-700 p-20 text-center space-y-4">
              <div className="w-16 h-16 bg-[#23233a] rounded-full flex items-center justify-center mx-auto text-slate-500"><Bell size={32}/></div>
              <h3 className="text-lg font-bold text-white">Framework empty</h3>
              <p className="text-slate-400">Official updates from GCE Erode will appear here.</p>
            </div>
          )}
        </div>
        ...
        {selectedNotif && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-[#1f1f2e] w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-slate-700">
              <div className="p-6 border-b flex justify-between items-start bg-[#23233a]">
                ...
                <button onClick={() => setSelectedNotif(null)} className="p-2 hover:bg-slate-800 rounded-full transition-colors"><XCircle size={24} className="text-slate-400"/></button>
              </div>
              <div className="p-8 overflow-y-auto flex-1 space-y-6">
                <div className="p-4 bg-cyan-950 rounded-2xl border border-cyan-900 flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#23233a] rounded-xl flex items-center justify-center text-cyan-400 font-bold border border-cyan-900">{selectedNotif.senderName.charAt(0)}</div>
                  <div>
                    <p className="font-bold text-cyan-300">{selectedNotif.senderName}</p>
                    <p className="text-xs font-bold text-cyan-400 uppercase">{selectedNotif.senderRole} • {selectedNotif.senderDepartment}</p>
                  </div>
                </div>
                <p className="text-slate-200 whitespace-pre-wrap leading-relaxed text-lg">{selectedNotif.content}</p>
                {selectedNotif.senderId === user?.id && (
                  <div className="pt-6 border-t border-slate-700 space-y-4">
                    <h4 className="font-bold text-white flex items-center gap-2"><Eye size={18}/> Delivery Tracking Summary</h4>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="p-4 bg-[#23233a] rounded-xl border border-slate-800 text-center">
                        <p className="text-[10px] font-bold text-slate-400 uppercase">Viewed</p>
                        <p className="text-xl font-bold text-cyan-400">{NotificationService.getNotificationStats(selectedNotif.id).viewed}</p>
                      </div>
                      <div className="p-4 bg-[#23233a] rounded-xl border border-slate-800 text-center">
                        <p className="text-[10px] font-bold text-slate-400 uppercase">Acknowledged</p>
                        <p className="text-xl font-bold text-emerald-400">{NotificationService.getNotificationStats(selectedNotif.id).acknowledged}</p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-2">
                        {NotificationService.getNotificationStats(selectedNotif.id).list.map((ack, i) => {
                          ...
                          return (
                            <div key={i} className="flex items-center justify-between p-3 bg-[#23233a] rounded-xl text-xs font-bold border border-slate-800">
                              <span className="text-slate-300">{identity || ack.userId}</span>
                              <span className={`px-2 py-0.5 rounded text-[8px] uppercase ${ack.status === 'ACKNOWLEDGED' ? 'bg-emerald-900 text-emerald-300' : 'bg-cyan-900 text-cyan-300'}`}>{ack.status}</span>
                            </div>
                          );
                        })}
                      </div>
