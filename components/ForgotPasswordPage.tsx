...
const ForgotPasswordPage: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
            <Link to="/login" className="absolute top-8 left-8 flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors font-semibold">
                <ArrowLeft size={20} />
                Back to Login
            </Link>
            ...
            <div className="w-full max-w-md space-y-8">
                ...
                <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-2xl shadow-slate-200/50">
                    {error && (
                        <div className="mb-6 p-4 bg-rose-50 text-rose-600 border border-rose-100 rounded-xl flex items-center gap-3">
                            <AlertCircle size={20} />
                    ...
                    {step === ResetStep.EMAIL && (
                        <form onSubmit={handleSendOTP} className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-700 ml-1 font-inter">institutional Email</label>
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full py-4 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
                            >
                    ...
                    {step === ResetStep.OTP && (
                        <form onSubmit={handleVerifyOTP} className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-700 ml-1 font-inter">6-Digit Code</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        required
                                        placeholder="000000"
                                        className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all tracking-[0.5em] font-mono text-center text-lg"
                                        value={otp}
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        required
                                        className="w-full pl-10 pr-12 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                                        value={passwords.new}
                    ...
                    {step === ResetStep.SUCCESS && (
                        <div className="text-center space-y-6">
                            ...
                            <p className="text-slate-600 font-medium">Password reset successful. Please login with your new credentials.</p>
                            <Link
                                to="/login"
                                className="block w-full py-4 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg active:scale-95 transition-all"
                            >
                                Go to Login
                            </Link>
                        </div>
